...
for vstrand in vstrands:
    for i in range(len(vstrand['stap'])):
        num = vstrand['num']
        if num+1>num_tot: num_tot = num+1
        ...
        if stap[0]!=stap[2]:
            if stap[0]>-1:
                if stap[2]>-1:
                    ...
                    #get the position of the staple that has a crossover point.

print('num of strands:', num_tot)
print('number of crossovers:', len(crossovers)/2)
#print('paired bp:', cnt_staple_bp)
...
for crossing in crossovers:
    ...
    if (paired_loc,location) not in staple_pairs: #need to remove the duplications from this list
        ...
        #print(staple_pairs[-1])

#for pair in staple_pairs: print(pair)
#print(staple_pairs)
...
#print(scaf_neighbors[start_position]) 
...
def distance_between(end_points, scaf_length):
    ...
    #distance = np.abs(end_points[1]-end_points[0])
    #if distance>scaf_length/2.: distance = scaf_length - distance
    return distance
